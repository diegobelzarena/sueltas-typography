...
def extract_pdf(
    ...
    doc.close()
    return f"OK: {pdf_stem} — {count} pages [PDF] (DPI {orig_dpi}→{target_dpi}, scale {scale:.2f})"
...
def extract_tiff_folder(
    ...
    if not tiff_files:
        return f"SKIP: {folder_name} — no TIFF files"
    ...
    for idx, tiff_path in enumerate(tiff_files):
        try:
            ...
            print(f"  Warning: {folder_name}/{tiff_path.name}: {e}")

    return f"OK: {folder_name} — {count} pages [TIFF] (DPI {orig_dpi}→{target_dpi}, scale {scale:.2f})"
...
def _convert_one(source: Path, out_dir: Path, target_dpi: int,
    ...
    if source.is_file() and source.suffix.lower() == ".pdf":
        ...
        return f"SKIP: {source.name} — unsupported type"


def _convert_wrapper(args: tuple) -> str:
  ...
  python scripts/convert_sources.py data/corpus-1/pdfs data/corpus-1/imgs --target-dpi 300
    ...
    if workers > 1 and len(tasks) > 1:
        ...
        for task in tqdm(tasks, desc="Converting"):
            try:
                results.append(_convert_wrapper(task))
            except Exception as e:
